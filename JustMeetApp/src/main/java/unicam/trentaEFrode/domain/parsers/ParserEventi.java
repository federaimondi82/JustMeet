...
public List<Evento> parseEventi(String newJson) {
		...
		for(int i = 0; i< agenda.length; i++) {
			...
			eventi.add(new Evento(
					evento[1], //nome
					new GregorianCalendar(Integer.parseInt(evento[2].split("/")[0]), Integer.parseInt(evento[2].split("/")[1]), Integer.parseInt(evento[2].split("/")[2]), Integer.parseInt(evento[3].split(":")[0]), Integer.parseInt(evento[3].split(":")[0])), // dataora
					Integer.parseInt(evento[4]), Integer.parseInt(evento[5]), //min max
					evento[6], //descr
					Integer.parseInt(evento[7]), //durata
					new Luogo(Integer.parseInt(evento[8]), evento[9], evento[10],evento[11],evento[12],evento[13],evento[14]),
					new Categoria(Integer.parseInt(evento[15]),evento[16], evento[17])
					).setId(Integer.parseInt(evento[0]))
			);
		}
		return eventi;
	}
