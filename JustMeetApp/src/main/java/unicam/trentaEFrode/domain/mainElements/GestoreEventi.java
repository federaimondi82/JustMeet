...
public class GestoreEventi extends Gestore{
	...
	public static GestoreEventi getInstance() {
		if(instance == null) instance = new GestoreEventi();
		return instance;
	}
	...
	/**
	 * Effettua i controlli necessari sull'evento passato e restituisce la lista dei codici 
	 * corrispondenti alle risposte dei controlli.
	...
	/**
	 * Controlla la validita' dell'oggetto dataOra, il quale deve corrispondere ad un tempo futuro.
	 * Nel caso di non validita', aggiunge alla lista passata, il codice corrispondene a tale errore.
	...
	private void controllaDataOra(GregorianCalendar dataOra, List<Integer> lista) {
		try {
			...
			if(dataOra.getTime().before(GregorianCalendar.getInstance().getTime())) throw new IllegalArgumentException();
	...
	/**
	 * Controlla la validit� dell'oggetto luogo, il quale deve avere tutti i campi validi.
	 * Nel caso di non validit�, aggiunge alla lista passata, il codice corrispondene a tale errore.
	...
	private void controllaLuogo(Luogo luogo, List<Integer> lista) {
		...
		if(indirizzo=="" || numeroCivico=="" || cap=="" || citta =="" || provincia=="") if(lista.indexOf(1)==-1) lista.add(1);
		else if(cap!= "" && !controllaCap(cap)) lista.add(2);
	}
	
	/**
	 * Controlla, tramite l'ordinamento naturale numerico, la validit� dei numeri min e max.
	 * Nel caso di non validit�, aggiunge alla lista passata, il codice corrispondene a tale errore.
	 * Il numero 0 corrisponde alla volont� da parte dell'utente di non specificare tale informazione.
	...
	private void controllaMinMax(Integer min, Integer max, List<Integer> lista ) {
		if(min!=0 && max!= 0 && min>max) lista.add(4);
	}
	...
	/**
	 * Modifica {@code evento} con i campi di {@code nuovo} solo se i campi di quest'ultimo sono validi.
	 * Se i campi non sono validi, vengono ignorati
	 * @throws ConnectException 
	 * */
	public List<Integer> modificaVeronica(Evento evento, Evento nuovo) throws ConnectException {
		...
		List<Integer> lista = new ArrayList<>();
		if(!evento.nome().equals(nuovo.nome()) & nuovo.nome()!= "") campiModificati.put("nome", nuovo.nome());
		if(!evento.descrizione().equals(nuovo.descrizione()) & nuovo.descrizione()!= "") campiModificati.put("descrizione", nuovo.descrizione());
		if(!evento.categoria().equals(nuovo.categoria()) & nuovo.categoria()!= null) campiModificati.put("categoria", ""+nuovo.categoria().getId() +"");
		if(!evento.dataOra().equals(nuovo.dataOra()) & nuovo.dataOra()!= null) {
			controllaDataOra(nuovo.dataOra(), lista);
			if(lista.size() == 0) campiModificati.put("dataOra", nuovo.dataOra().toString());
			else lista.clear();
		}
		if(!evento.luogo().equals(nuovo.luogo()) & nuovo.luogo()!= null) {
			controllaLuogo(nuovo.luogo(), lista);
			if(lista.size() == 0) campiModificati.put("luogo", nuovo.luogo().toString());
			else lista.clear();
		}
		if(evento.minPartecipanti() != nuovo.minPartecipanti() || evento.maxPartecipanti() != nuovo.maxPartecipanti()) {
			controllaLuogo(nuovo.luogo(), lista);
			...
			if(campiModificati.size()!= 0) lista.add(this.registratore.modificaVeronica(nuovo.id(), campiModificati)?-1:0);
		return lista;
	}

	/**
	 * Incarica il backend ad inviare una notifica a tutti i partecipanti dell'evento passato, contenente
	 * il testo passato
	 * */
